	...
	uint32_t num_streams, unsigned char *endpoints, int num_endpoints)
{
	...
	r = ioctl(fd, (unsigned int )req, streams);
...
static void op_clear_transfer_priv(struct usbi_transfer *itransfer)
{
	...
	case LIBUSB_TRANSFER_TYPE_INTERRUPT:
		if (tpriv->urbs) {
			/* Zero usercontext before freeing so that any stale kernel URBs
			 * returned via REAPURBNDELAY after this point will have a NULL
			 * usercontext, allowing reap_for_handle to discard them safely.
			 * Without this, freed memory may retain the old non-NULL value and
			 * bypass the !tpriv->urbs guard in handle_bulk_completion on
			 * Android < 16 (no MTE). */
			for (int i = 0; i < tpriv->num_urbs; i++)
				tpriv->urbs[i].usercontext = NULL;
			free(tpriv->urbs);
			tpriv->urbs = NULL;
		}
		break;
	...
	struct usbfs_urb *urb)
{
	 ...
	 *
	 * __EBCANDROID__: the NULL check is deliberately made INSIDE itransfer->lock.
	 * Reading tpriv->urbs unlocked leaves a TOCTOU window -- another thread can free it
	 * between the test and the urb_idx arithmetic below, which is exactly the
	 * use-after-free this guard exists to prevent. The RTL_SDR_AIS_Driver copy that the
	 * rest of this file comes from checks before locking; the rtlsdr433 copy has the
	 * safer ordering used here. Union of both. */
...
completed:
	/* Zero usercontext in every URB before freeing: if the kernel later
	 * returns a stale reap for one of these (REAP_AFTER_DISCONNECT race),
	 * reap_for_handle will see NULL and safely discard it. */
	for (int _i = 0; _i < tpriv->num_urbs; _i++)
		tpriv->urbs[_i].usercontext = NULL;
	free(tpriv->urbs);
	tpriv->urbs = NULL;
	usbi_mutex_unlock(&itransfer->lock);
...
static int reap_for_handle(struct libusb_device_handle *handle)
{
	...
	/* Defensive guard: usercontext should never be NULL for a properly
	 * submitted URB, but if a stale kernel URB from a freed transfer
	 * is reaped (e.g. REAP_AFTER_DISCONNECT race on Android < 16),
	 * the zeroing in op_clear_transfer_priv / handle_bulk_completion
	 * ensures we see NULL here instead of a dangling pointer. */
	if (!itransfer) {
		usbi_err(HANDLE_CTX(handle),
			"reaped URB has NULL usercontext — stale URB, discarding");
		return 0;
	}
